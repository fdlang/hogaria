...
export function PublicLanding({ api, onLogin }: Props) {
  ...
  return <div className="portfolio">
    ...
    <section id="obras" className="section"><header><span className="eyebrow">Obras seleccionadas</span><h2>El detalle no es<br /><em>un extra.</em></h2><p>Cada espacio parte de una conversación y termina con una casa que funciona mejor.</p></header><div className="works">{portfolioWorks.map(({ title, type, image }) => {
      ...
      return <article key={title}><img src={image} alt={title} width="1200" height="800" loading="lazy" decoding="async" /><div><small>{type}</small><h3>{title}</h3><button className={isBathroomCase ? "works__case-link" : undefined} onClick={() => jump(isBathroomCase ? "proceso" : "contacto")} aria-label={isBathroomCase ? "Ver el caso de estudio Baño Pinto" : `Solicitar un proyecto como ${title}`}>{isBathroomCase ? "Ver proyecto →" : "↗"}</button></div></article>;
    })}</div></section>
