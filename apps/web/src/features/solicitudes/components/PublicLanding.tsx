  ...
  return (
    <section id="proceso" className="project-showcase" aria-labelledby="bano-pinto-title">
      ...
      <div className="project-showcase__materials">
        ...
        <h3>Porcelánico de gran formato, luz integrada y acabados pensados para durar.</h3>
...
export function PublicLanding({ api, onLogin }: Props) {
  ...
  return <div className="portfolio">
    ...
    <section className="hero" id="inicio"><div><span className="eyebrow">Hogaria · Madrid</span><h1>Espacios que<br /><em>merecen</em> la pena vivir.</h1><p>Reformas e interiorismo con diseño, planificación y ejecución bajo control. Sin sorpresas, con mucho oficio.</p><div className="actions"><Button onClick={() => jump("contacto")}>Cuéntanos tu proyecto</Button><button onClick={() => jump("obras")}>Ver obras seleccionadas</button></div></div><img className="hero-marble" src="/brand/hogaria-hero.jpg" alt="" width="1920" height="1280" decoding="async" /><span className="stamp"><b>+</b>diseño<br />con oficio</span></section>
    <section id="obras" className="section"><header><span className="eyebrow">Obras seleccionadas</span><h2>El detalle no es<br /><em>un extra.</em></h2><p>Cada espacio parte de una conversación y termina con una casa que funciona mejor.</p></header><div className="works">{portfolioWorks.map(({ title, type, image }) => {
      const isBathroomCase = title === "Baño Pinto";
      return <article key={title}><img src={image} alt={title} width="1200" height="800" loading="lazy" decoding="async" /><div><small>{type}</small><h3>{title}</h3><button className="works__action" onClick={() => jump(isBathroomCase ? "proceso" : "contacto")} aria-label={isBathroomCase ? "Ver el caso de estudio Baño Pinto" : `Solicitar un proyecto como ${title}`}>{isBathroomCase ? "Ver proyecto" : "Hablar del proyecto"}</button></div></article>;
    })}</div></section>
    <section id="servicios" className="section dark">
      <header>
        <div><span className="eyebrow">Lo que hacemos</span><h2>Una reforma,<br /><em>bien pensada.</em></h2></div>
        <p>De la primera idea a la entrega final: un proyecto claro, materiales elegidos con criterio y una ejecución cuidada en cada detalle.</p>
      </header>
