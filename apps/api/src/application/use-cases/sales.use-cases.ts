...
export class EstimateUseCases {
  ...
  async reject(actorId: number, id: number, motivo: string, ctx: ClientContext) { const actor = await this.users.findById(actorId); const estimate = await this.require(id); if (!actor || actor.rol !== "cliente" || actor.id !== estimate.clienteId) throw new ForbiddenError(); if (estimate.estado !== "enviado") throw new ConflictError("Solo se puede rechazar una propuesta pendiente"); if (!motivo.trim()) throw new ValidationError("Indica el motivo o los cambios solicitados", "motivo"); const saved = await this.estimates.update(id, { estado: "rechazado" }); await this.events.emit({ type: "EstimateRejected", eventId: crypto.randomUUID(), occurredAt: new Date(), actorId: actor.id, actorName: actor.nombre, ip: ctx.ip, userAgent: ctx.userAgent, estimateId: id, motivo: motivo.trim() }); return saved; }
  ...
  async sign(actorId: number, id: number, input: { password: string; canvasSignature: string; consentimiento: string }, ctx: ClientContext) { const actor = await this.users.findById(actorId); const estimate = await this.require(id); if (!actor || actor.rol !== "cliente" || actor.id !== estimate.clienteId) throw new ForbiddenError(); if (estimate.estado !== "enviado") throw new ConflictError("La propuesta no está pendiente de firma"); if (!input.password || !input.canvasSignature || !input.consentimiento?.trim()) throw new ValidationError("Firma, contraseña y consentimiento son obligatorios"); const verified = await this.users.verifyPassword(actor.email.value, input.password); if (!verified || verified.id !== actor.id) throw new ForbiddenError("No se ha podido verificar tu identidad"); const versions = await this.estimates.findVersions(id); const current = versions.find(v => v.version === estimate.versionActual); if (!current || current.firmadoAt) throw new ConflictError("Esta versión no está disponible para firmar"); const now = new Date(); const hash = await this.crypto.hashDocument(JSON.stringify({ estimateId: estimate.id, version: current.version, snapshot: current.snapshot })); const token = await this.crypto.generateSignatureToken(estimate.id, actor.id, now.getTime()); const firma = { firmante: actor.nombre, email: actor.email.value, ip: ctx.ip, userAgent: ctx.userAgent, consentimiento: input.consentimiento.trim(), canvasSignature: input.canvasSignature, hash: hash.value, token, fechaFirma: now.toISOString() }; await this.estimates.signVersion(id, current.version, firma, now); const saved = await this.estimates.update(id, { estado: "firmado" }); await this.events.emit({ type: "BudgetSigned", eventId: crypto.randomUUID(), occurredAt: now, actorId: actor.id, actorName: actor.nombre, ip: ctx.ip, userAgent: ctx.userAgent, budgetId: id, hash: hash.value }); return { estimate: saved, hash: hash.value, fechaFirma: now }; }
  async accept(actorId: number, id: number, ctx: ClientContext): Promise<Project> { const actor = await this.users.findById(actorId); assertAdmin(actor); const estimate = await this.require(id); if (estimate.estado !== "firmado") throw new ConflictError("Solo una propuesta firmada puede convertirse en proyecto"); const versions = await this.estimates.findVersions(id); const current = versions.find(v => v.version === estimate.versionActual); if (!current?.firmadoAt) throw new ConflictError("Falta la firma de la versión vigente"); const opportunity = await this.opportunities.findById(estimate.oportunidadId); if (!opportunity) throw new NotFoundError("Oportunidad"); const subtotal = current.snapshot.partidas.reduce((sum, l) => sum + l.cantidad * l.precioVentaUnitario * (1 - l.descuento / 100), 0); const project: Project = { id: 0, estimateId: estimate.id, nombre: estimate.titulo, descripcion: opportunity.descripcion, clienteId: estimate.clienteId, direccion: opportunity.direccion, tipo: opportunity.tipo, estado: "planificacion", progreso: Percentage.zero(), presupuesto: Money.of(subtotal), fechaInicio: new Date(), fechaFinPrevista: new Date(), profesionalesAsignados: [], hitos: [], createdAt: new Date() }; const saved = await this.projects.save(project); await this.estimates.update(id, { estado: "aceptado" }); await this.opportunities.update(opportunity.id, { estado: "ganada" }); await this.events.emit({ type: "EstimateAccepted", eventId: crypto.randomUUID(), occurredAt: new Date(), actorId: actor.id, actorName: actor.nombre, ip: ctx.ip, userAgent: ctx.userAgent, estimateId: id, projectId: saved.id }); return saved; }
  ...
  private async publicView(estimate: Estimate) { const versions = await this.estimates.findVersions(estimate.id); const version = versions.find(v => v.version === estimate.versionActual) ?? null; const snapshot = version?.snapshot ?? null; const subtotal = snapshot ? snapshot.partidas.reduce((sum, line) => sum + line.cantidad * line.precioVentaUnitario * (1 - line.descuento / 100), 0) : null; const iva = snapshot && subtotal != null ? snapshot.partidas.reduce((sum, line) => sum + line.cantidad * line.precioVentaUnitario * (1 - line.descuento / 100) * line.iva / 100, 0) : null; return { id: estimate.id, numero: estimate.numero, titulo: estimate.titulo, estado: estimate.estado, versionActual: estimate.versionActual, createdAt: estimate.createdAt, updatedAt: estimate.updatedAt, propuesta: snapshot ? { ...snapshot, totalSinIva: subtotal, totalIva: iva, totalConIva: subtotal! + iva!, enviadoAt: version!.enviadoAt, firmadoAt: version!.firmadoAt, hash: typeof version!.firma?.hash === "string" ? version!.firma.hash : null } : null }; }
}
